...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  // Coerce the COUNT result to a number to satisfy TS
  const rowsArr = (result as any)?.rows ?? [];
  const cnt = Number(rowsArr[0]?.cnt ?? 0);
  const alreadyHasBaseline = cnt >= 4;
  if (alreadyHasBaseline) {
    payload?.logger?.info?.('[20251018_124259] Baseline tables already present — skipping idempotently.');
    return;
  }
  ...
  CREATE TYPE "public"."enum_users_roles" AS ENUM('admin', 'manager', 'trainer');
